...
new_survey <- FALSE
use_survey_path <- "survey_2021_11_01"
user_survey <- file.path(basedir,use_survey_path)
...
elevatr_wrapper_for_tile <- function(tile_id) {
  ...
  raster::writeRaster(elevatr_NED,filename=file.path(snapshot_dir,paste0('tile_',tile_id),"NED_4326.tif"),bylayer=TRUE,format="GTiff")
  gdalUtilities::gdalwarp(srcfile = file.path(snapshot_dir,paste0('tile_',tile_id),"NED_4326.tif"), dstfile = file.path(snapshot_dir,paste0('tile_',tile_id),"NED.tif"), t_srs = sf::st_crs(6349), r = "bilinear")   
  
  #Toss tmp ned file.
  file.remove(file.path(snapshot_dir,paste0('tile_',tile_id),"NED_4326.tif"))
...
# id <- 88748
# dir.create(file.path(snapshot_dir,paste0('tile_',id)), showWarnings = FALSE)
# elevatr_wrapper_for_tile(id)
# breaks: 79647
##############################################################################################################################
##############################################################################################################################
...
# mosaic_bathy_terrain_surface(id)
##############################################################################################################################
##############################################################################################################################
