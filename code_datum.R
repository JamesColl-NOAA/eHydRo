...
xx$eHydro_survey_footprints <- sf::read_sf(paste0(basedir, "/base_data/eHydro_Survey_Data/SurveyJob.shp")) %>%
  sf::st_transform(sf::st_crs(5499))
##############################################################################################################################
##############################################################################################################################
...
xx$survey_tiles <- sf::st_make_grid(bbox_meters, cellsize = 10000, what = "polygons", square = FALSE, flat_topped = TRUE) %>%
  sf::st_as_sf()
xx$survey_tiles <- sf::st_transform(xx$survey_tiles, sf::st_crs(5499))
...
elevatr_wrapper_for_tile <- function(tile_id) {
  ...
  gdalUtilities::gdalwarp(srcfile = file.path(basedir,"survey",paste0('tile_',tile_id),"NED_4326.tif"), dstfile = file.path(basedir,"survey",paste0('tile_',tile_id),"NED.tif"), t_srs = sf::st_crs(5499), r = "bilinear")   
...
mosaic_bathy_surface <- function(AOI_tiles_with_paths,tile_id) {
  ...
  for (x in 1:length(distinct_surveys)) {
    ...
    if(TRUE) {
      ...
      if(is.character(pts)) {
        tryCatch({
          ...
          tmp_pts1 <- sf::st_transform(sf::st_as_sf(tmp_pts1), sf::st_crs(5499))
          tmp_pts1 <- tmp_pts1 %>%
    ...
    pts <- sf::st_as_sf(data.frame(tmp_pts), coords = c("tmp_pts1.X", "tmp_pts1.Y", "tmp_pts1.Z"), crs = sf::st_crs(5499))
    ...
    gdalUtilities::gdalwarp(file.path(basedir,"survey",paste0('tile_',dst_folder),"tmp",paste0("BATHY_mask",x,".tif")), dstfile = file.path(basedir,"survey",paste0('tile_',dst_folder),"tmp",paste0("BATHY_proj",x,".tif")), t_srs = sf::st_crs(5499), r = "bilinear") 
  }
...
#)
mosaic_bathy_surface(elemnts1,180)
dst_folder <- 180
##############################################################################################################################
mosaic_bathy_terrain_surface <- function(tile_id) {
  ...
  ba
  bathy_ras <- raster::setExtent(interp_ras,AOI_tiles_with_paths,TRUE,TRUE)
...
cut_cross_sections <- function(tile_id,cross_section_locations) {
  ...
  step_run_time <- difftime(Sys.time(), step_start_time, units = "mins")
  print(paste0("Cross section database written out to" ,," in ",round(step_run_time, digits = 2), " minutes"))
